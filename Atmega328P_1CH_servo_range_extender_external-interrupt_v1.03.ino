...
void loop() {
  ...
  if (calibrating) {
    ...
    if (lastButtonState == HIGH && buttonState == LOW) {
      ...
      currentStep++;
      calibrationLock = millis();
      if (currentStep > 2) { quickBlink(5); currentStep = 0; calibrating = false; delay(1000); longBlink(3); saveCalibration(); }
    }
    lastButtonState = buttonState;
  }
  else {
